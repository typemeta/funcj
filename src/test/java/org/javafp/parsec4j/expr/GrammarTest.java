...
public class GrammarTest {
    ...
    private static void assertSuccess(String s, String expected) throws Exception {
        final String result = Grammar.parser.run(Input.of(s)).getOrThrow().toString();
        Assert.assertEquals(expected, result);
        Assert.assertEquals(expected, Grammar.parser.run(Input.of(result)).getOrThrow().toString());
    }

    private static void assertFailure(String s) {
        Grammar.parser.run(Input.of(s)).match(
            ok -> {
